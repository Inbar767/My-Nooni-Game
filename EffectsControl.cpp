...
EffectsControl::EffectsControl(gameDataRef data) : 
	data(data), currentEffect(EffectsControl::EffectType::NO_EFFECT), countAnimation(0)
{
	addEffects();
}
...
{
	if (currentEffect != EffectsControl::EffectType::NO_EFFECT)
	{
...
void EffectsControl::update()
{
	if (currentEffect != EffectsControl::EffectType::NO_EFFECT)
	{
		if(effects.at(currentEffect)->animation() && currentEffect != EffectsControl::EffectType::PET_EFFECT)
		{
			countAnimation++;
			if (countAnimation == maxAnimation) //If current effect is animated maxAnimation times - stop effect
			{
				currentEffect = EffectsControl::EffectType::NO_EFFECT;
				countAnimation = 0;
			}
		}
	}
...
//Playes effect
void EffectsControl::startEffect(EffectsControl::EffectType effectNumber)
{
	int effectIndex = static_cast<int>(effectNumber);
	if (effectNumber >= EffectsControl::EffectType::SLEEP_EFFECT && effectIndex < effects.size())
	{
		currentEffect = effectNumber;
		countAnimation = 0;
		//Changes maxAnimation accordding to type of the effect
		if (effectNumber == EffectsControl::EffectType::SLEEP_EFFECT)
		{
			maxAnimation = 5;
		}
		else
		{
			if (effectNumber == EffectsControl::EffectType::BATH_EFFECT)
			{
				maxAnimation = 2;
			}
			else
...
{
	return (currentEffect != EffectsControl::EffectType::NO_EFFECT);
}
